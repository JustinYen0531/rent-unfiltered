...
- 第一版不應承諾所有平台皆能完美辨識
- 消防設備、逃生動線、違法隔間、修繕責任、可報稅與可遷戶籍等 RRI 關鍵因素，常常無法只靠平台頁面確認

因此較穩妥的產品表述是：

插件會協助擷取使用者目前頁面的可見文字，並盡可能辨識租屋相關欄位，作為 RHIR 與透明度分析的輸入來源。

若插件或平台文字無法確認某些 RRI 關鍵欄位，系統不應自動推定是或否，而應：

- 在 RHIR 中將欄位標示為 `missing`
- 在 `transparencyLayer.fieldsNeedingUserQuestion` 記錄欄位 key
- 在 `transparencyLayer.followupQuestions` 產生使用者可執行的追問或現場確認問題
- 在 UI 上同步顯示這些問題，讓使用者知道下次看房、問房東或檢查契約時要補什麼
