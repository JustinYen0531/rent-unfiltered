...
function RiskPill({ score, level }) {
  if (score == null) return <span className="risk-pill risk-mid" style={{background: "#eceef1", color: "#5a6573"}}>未分析</span>;
  ...
  return (
    <span className={`risk-pill ${cls}`}>
      ...
      <span style={{opacity:0.75}}>· {level}</span>
    </span>
  );
}
      ...
      <header className="topbar">
        ...
        <div className="topbar-right">
          <div className="search">
            ...
            <input placeholder="搜尋紀錄、地址、版本…"/>
            <span className="kbd mono">⌘K</span>
          </div>
          <button className="btn btn-ghost" title="通知"><Icon name="bell" size={16}/></button>
...
function GovFoot() {
  return (
    <footer className="gov-foot">
      <div>RENT UNFILTERED · 開放給第一次租屋者的資訊透明專案</div>
...
window.RU = Object.assign(window.RU || {}, {
  Icon, Badge, RiskPill, TopBar, Crumbs, GovFoot, highlightJSON,
});
